  ...
  if (q.type === 'choice') {
    ...
    if (!isPlainObject(criteria)) {
      ...
      for (const name of names) {
        ...
        if (desc === null || desc === undefined) undescribed.push(name);
      }
      // 合并成一条：自然语言选项列表（「选项 甲、乙、丙」）会一次给出 4 个无描述的选项，
      // 逐个报会让 4 条一模一样的警告把真正的问题淹掉。
      if (undescribed.length) {
        warnings.push(
          `${undescribed.length} 个选项没有描述（${undescribed.join('、')}）：Jev 官方建议「描述性 criteria 比光秃秃的标签要好判」，` +
            '分流/归类任务里描述尤其重要——它决定了模型怎么划边界。标签本身够清楚时才可以省。',
        );
      }
