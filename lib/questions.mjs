...
const NOUL_HINT = /(是否|是不是|正确吗|对不对|成立(吗|么)?|真的|能否|能不能|可不可以|符合.{0,4}吗|属于.{0,4}吗)/;
...
export function customQuestion({ text, domain, type = null, maxOptions = LIMITS.choice.max, scoreSet = null }) {
  ...
  if (resolved === 'noul') {
    ...
    notes.push('判断题默认附带 true/false 判据，明确「什么算成立」，减少模型对命题边界的自由解读。');
  }
 ...
 *
 * @param {{domainKey?:string, input:string, userQuestion?:string, types?:string[], maxOptions?:number, includeTemplates?:boolean}} args
 */
export function planQuestions({
  ...
  const apt = userQuestion ? checkTruthApt(userQuestion, domain) : { ok: true, status: 'unknown', reasons: [], reformulations: [] };
  if (userQuestion && !apt.ok) {
    notes.push('⚠️ 用户问题不是「有真值」的问题，已给出可回答的改写建议（见 truth_apt）。');
  }

  if (userQuestion) {
    const custom = customQuestion({ text: userQuestion, domain, type: null, maxOptions });
    if (custom.ok) {
      candidates.push({ ...custom.question, origin: 'user' });
      notes.push('用户自定义问题通过了结构校验，已作为候选之一。');
    } else {
      notes.push(`用户自定义问题未能编译：${custom.reason} ${custom.hint || ''}`.trim());
    }
  }
