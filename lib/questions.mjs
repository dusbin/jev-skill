...
/**
 * 自然语言选项列表：`选项 物流、退款、产品故障、其他` / `类别：A、B、C` / `options: x, y`。
 *
 * ## 为什么需要这条路
 *
 * 用 Jev 的人最自然的写法是「我的订单三天没发货了 选项 物流、退款、产品故障、其他」——
 * 这是人给机器指定「可选集合」的方式，也正是 Jev 官方 criteria map 的文字版。
 * 但只认 `A. … B. …` 的抽取器对这种写法完全无感，结果是**用户按 Jev 的用法提问，工具却说抽不到选项**。
 *
 * ## 三重守卫（宁可抽不到，也不要抽错）
 *
 * 1. 必须先出现显式的列表标记词（选项/可选/类别/分类/标签/枚举/options），
 *    不能是句子里的普通顿号并列——否则「物流、退款、产品故障」这种短语会被当成三个选项；
 * 2. 切出来的每一项都必须是**短标签**（≤16 字）且不含句末标点，否则那是句子不是选项；
 * 3. 2–8 项、互不重复。超出上限的由 `validateQuestion` 按 `--max-options` 报错，这里不截断。
 *
 * @returns {Array<{label:string, text:null, raw:string}>} 抽不到返回空数组
 */
/** 自然语言选项列表的匹配（供抽取与剥离两处共用，避免两条正则各自漂移）。 */
const OPTION_LIST_RE = /(?:^|[\s，。；:：])(选项|可选|候选|类别|分类|标签|枚举|options?)\s*[:：]?\s*([^\n。！？]{2,160})/i;
 ...
 * ## 为什么必须去掉
 *
 * `state` 是「要被判断的内容」。如果选项列表留在 state 里，会出现一个很隐蔽的污染：
 * 用户写「我的订单三天没发货了 选项 物流、**退款**、产品故障、其他」，
 * 而 state 里就出现了「退款」二字——判断时**极易把它当成客户说过的话**，
 * 从而把一条物流工单误分到退款队列。选项是**给工具的指令**，不是客户的内容。
 *
 * 但**标记式选项**（`A. … B. …`）不能剥：那是题干的一部分
 * （Jev 官方示例里 state 就是完整题目），剥了题目就不完整了。
 * 所以只在「走自然语言列表这条路径」时剥离。
 */
export function stripOptionList(text) {
  const t = String(text ?? '');
  if (extractOptionMarkers(t).length >= 2) return t;
  const m = t.match(OPTION_LIST_RE);
  if (!m) return t;
  return squeeze(`${t.slice(0, m.index)} ${t.slice(m.index + m[0].length)}`);
}

export function extractOptionList(text) {
  const t = String(text ?? '');
  const m = t.match(OPTION_LIST_RE);
  if (!m) return [];
  const body = m[2].trim();
  // 带「名字=描述」时，分隔符只能用分号/换行——因为顿号此时是**描述内部**的分隔
  //（「物流=发货、出库、运输」）。不带等号时才用顿号切。
  const hasDesc = /[=＝]/.test(body);
  const itemSep = hasDesc ? /[;；\n]+/ : /[、，,;；|/]+/;
  ...
  const parsed = items.map((item) => {
    if (!hasDesc) return { label: item, text: null, raw: item };
    const at = item.search(/[=＝]/);
    const name = (at === -1 ? item : item.slice(0, at)).trim();
    const desc = at === -1 ? '' : item.slice(at + 1).trim();
    return { label: name, text: desc || null, raw: item };
  });

  if (parsed.length < 2 || parsed.length > 8) return [];
  // 名字必须是短标签；描述可以长一些，但也不能长到像句子
  if (parsed.some((o) => o.label.length === 0 || o.label.length > 16)) return [];
  if (parsed.some((o) => (o.text || '').length > 60)) return [];
  if (items.some((x) => /[。！？!?]/.test(x))) return [];
  if (new Set(parsed.map((o) => o.label)).size !== parsed.length) return [];
  // text 给 null 而不是编一个：「物流类」这种填充描述会让产物看起来像有描述，
  // 而 Jev 的 criteria 描述是要拿去做判别依据的。宁可用 null（Jev 允许），并让校验器发出警告。
  return parsed;
}
...
export function extractOptions(text) {
  const t = String(text ?? '');
  // 标记式路径的**所有**「抽不到 / 抽得不合法」出口都必须落到自然语言列表这条兜底路径上。
  // 之前这里是一串 `return []`，兜底代码写在最后一个 return 之后——**永远到不了**，
  // 于是「选项 物流、退款、产品故障、其他」这种最自然的写法仍然抽不到选项。
  const viaMarkers = extractOptionMarkers(t);
  if (viaMarkers.length >= 2) return viaMarkers;
  return extractOptionList(t);
}

/** 标记式抽取（`A. … B. …` / `(A) …` / `① …` / `1. …`）。不合法时返回空数组。 */
function extractOptionMarkers(t) {
  const matches = [];
...
export function planQuestions({
  ...
  const options = extractOptions(input);
  // 选项一旦抽出来进了 criteria，就不该再留在 state 里污染判断（标记式选项除外，见 stripOptionList）
  const state = stripOptionList(plainText(input) || squeeze(input));
  const notes = [];
  const candidates = [];
