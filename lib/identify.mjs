...
export const IDENTIFY_TUNING = Object.freeze({
  ...
  tie_ratio: 0.15,
  /**
   * 证据量下限。低于它一律判 ask，不看置信度。
   *
   * 为什么需要这条：**「没有竞争对手」不等于「我很确信」**。
   * 五项权重里 share（优势占比）与 margin（领先幅度）都在「只有一家得分」时拉满，
   * 于是**一条弱词（1 分）就能算出 0.743 的置信度并自动采用**。
   * 实测输入「乌克兰无人机袭击俄罗斯莫斯科Kapotnya区的最新图像。」正是因为
   * 只有英语的「中英混排」模式命中 1 分，被判成了英语——而它明显是时政。
   *
   * 2.5 的含义：至少一条强证据（3 分），或两条中/弱证据（2+1）。
   * 提高它会更容易回问用户，降低它会让单条弱词重新有机会定案。
   */
  min_evidence_mass: 2.5,
});
...
export function identifyDomain({ text = '', hint = '' } = {}) {
  ...
  } else if (mass < IDENTIFY_TUNING.min_evidence_mass) {
    decision = 'ask';
    notes.push(
      `证据总量只有 ${mass} 分，低于下限 ${IDENTIFY_TUNING.min_evidence_mass}：` +
        `命中「${top.hits.map((h) => h.term).join('」「')}」这种单条证据不足以定案` +
        '（「没有竞争对手」不等于「确信」），请用户确认领域。',
    );
