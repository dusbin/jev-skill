...
|---|---|
| 一个编号（1/2/3） | `--pick <n>` |
| 多个编号（1 和 3） | `--pick 1,3` —— **推荐**：Jev 的核心建议就是「同一 state 的问题放在一次请求里」，批量问比分次问便宜也更快 |
| 自己的问题 + 某个候选 | `--custom "…" --pick 3`（注意：给了 `--custom` 后你的问题会排在候选第 1 条，预定义候选序号整体后移） |
...
| `--pick <a,b>` | 一次请求问多个问题 | 想同时拿到「这个说法准不准」和「具体到某个细节对不对」时 |

### ⚠️ 看到 `decision: "ask"` 时先看是哪一道闸门

`ask` 有三个来源，处理方式不同：

| notes 里的话 | 含义 | 怎么办 |
|---|---|---|
| 「没有任何领域词命中」 | 输入太短/太抽象 | 问用户领域 |
| 「证据总量只有 N 分，低于下限 2.5」 | 命中了，但只有一两条弱证据。**「没有竞争对手」不等于「确信」** | **先怀疑词典**：很可能是新词/新领域，词典没覆盖。把这个词记下来，问用户确认领域后考虑补进 `lib/domains/*.mjs` |
| 「识别置信度 X 低于阈值 0.55」 | 有相当证据但两家咬得紧 | 把候选领域抛给用户 |

第二条最值得留意：**它出现的频率直接反映词典覆盖率**。如果是新领域，补词表比重问用户更值。
